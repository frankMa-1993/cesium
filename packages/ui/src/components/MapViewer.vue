<template>
  <div class="map-viewer">
    ...
    <!-- 自定义气泡窗口 -->
    <div v-if="popupVisible" class="cesium-popup" :style="popupStyle">
...
const layerDdRef = ref<HTMLDivElement>()
let viewer: Cesium.Viewer | null = null
let pointLayer: any = null
let waterPrimitive: Cesium.Primitive | null = null
let gaodeImageryLayer: Cesium.ImageryLayer | null = null
...
const popupData = ref<Record<string, any>>({})
...
const statusText = computed(() => {
  const map: Record<string, string> = { online: '正常', warning: '预警', danger: '告警' }
...
function setWaterLayer(show: boolean) {
  if (!viewer) return
  if (show) {
...
function setGaodeLayer(show: boolean) {
  if (!viewer) return
  if (show) {
...
async function initMap() {
  ...
  if ((res as any).code === 200 && (res as any).data) {
    pointLayer = loadBillboardPoints(viewer, (res as any).data, onPointClick)
  }
}

async function onPointClick(entity: Cesium.Entity, cartesian: Cesium.Cartesian3) {
  const id = entity.id as string
  const res = await fetchPointDetail(id)
  if ((res as any).code === 200 && (res as any).data) {
    popupData.value = (res as any).data
