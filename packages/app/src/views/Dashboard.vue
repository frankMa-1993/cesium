      ...
      <aside class="panel-left">
        <div class="data-panel">
          ...
          <div class="stats-grid">
            <StatCard label="设备在线数" :value="overview.onlineDevices" unit="台" color="#00f0ff" />
            <StatCard label="设备总数" :value="overview.totalDevices" unit="台" color="#007aff" />
            <StatCard label="在线率" :value="overview.onlineRate" unit="%" color="#52c41a" />
            <StatCard label="今日任务" :value="overview.todayTask" unit="项" color="#ff9c00" />
          </div>
        </div>
      ...
      <aside class="panel-right">
        <div class="data-panel">
          ...
          <div class="stats-grid cols-2">
            <StatCard label="严重告警" :value="overview.alertCount" unit="条" color="#ff4d4f" />
            <StatCard label="一般预警" :value="overview.warningCount" unit="条" color="#ff9c00" />
          </div>
        </div>

        <div class="data-panel">
          <div class="panel-title">在线趋势</div>
          <TrendChart :data="trend.onlineTrend" :labels="trend.hours" :width="260" :height="100" />
        </div>

        <div class="data-panel">
          <div class="panel-title">告警趋势</div>
          <TrendChart :data="trend.alertTrend" :labels="trend.hours" :width="260" :height="100" color="#ff4d4f" />
        </div>
...
const trend = ref({
  ...
  alertTrend: [] as number[],
  dataVolume: [] as number[],
})
...
async function loadData() {
  try {
    ...
    if ((oRes as any).code === 200) overview.value = (oRes as any).data
    if ((aRes as any).code === 200) alerts.value = (aRes as any).data.list
    if ((tRes as any).code === 200) trend.value = (tRes as any).data
  } catch (e) {
